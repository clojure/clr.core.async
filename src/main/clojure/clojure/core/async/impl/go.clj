...
(ns ^{:skip-wiki true}
  clojure.core.async.impl.go
  (:require [clojure.tools.analyzer.ast :as ast]
            [clojure.tools.analyzer.env :as env]
            [clojure.tools.analyzer.passes :refer [schedule]]
            [clojure.tools.analyzer.passes.clr.annotate-loops :refer [annotate-loops]]            ;;; .jvm
            [clojure.tools.analyzer.passes.clr.warn-on-reflection :refer [warn-on-reflection]]    ;;; .jvm
            [clojure.tools.analyzer.clr :as an-clr]                                               ;;; .jvm  an-jvm
  ...
  (:import [clojure.core.async.impl.mutex ILock]                                                  ;;; [java.util.concurrent.locks Lock]
           [clojure.core.async.impl.concurrent IAtomicReferenceArray]                             ;;; [java.util.concurrent.atomic AtomicReferenceArray]
           [clojure.lang Var]))
...
(defn print-plan []
  (fn [plan]
    ((requiring-resolve 'clojure.pprint/pprint) plan)
    [nil plan]))
                       ...
                       (if finally
                         (gen-plan
                           [cur-blk (get-block)
                            ...
                            ;; rt/EXCEPTION-FRAMES.  every try/catch pushes at
                            ;; least 1 frame on to rt/EXCEPTION-FRAMES,
...
(defmethod -item-to-ssa :record
  [x]
  (-item-to-ssa `(~(symbol (.FullName (class x)) "create")                 ;;; .getName
...
(defn- emit-state-machine [machine num-user-params custom-terminators]
  (let [index (index-state-machine machine)
    ...
    `(fn state-machine#
	   ([] (rt/aset-all! (conc/->AtomicReferenceArray (object-array (int ~state-arr-size)))                                    ;;; (AtomicReferenceArray. ~state-arr-size)  -- use an Array for now  TODO -- fix!
...
(defn finished?
  ...
  [state-array]
  (identical? (rt/aget-object state-array rt/STATE-IDX) ::finished))
...
(defn mark-transitions
  {:pass-info {:walk :post :depends #{} :after an-clr/default-passes}}                  ;;; an-jvm
  [{:keys [op fn] :as ast}]
...
(def passes (into (disj an-clr/default-passes #'warn-on-reflection)              ;;; an-jvm
              #{#'propagate-recur
...
(defn emit-hinted [local tag env]
  (let [tag (or tag (-> local meta :tag))
    ...
    (if-let [prim-fn (case (cond-> tag (string? tag) symbol)
                       int `int       uint `uint                                 ;;; DM Added cases
...
(defn state-machine [body num-user-params [crossing-env env] user-transitions]
  (binding [an-clr/run-passes run-passes]                                        ;;; an-jvm
    (-> (an-clr/analyze `(let [~@(if (nested-go? env)                            ;;; an-jvm
          ...
          {:passes-opts (merge an-clr/default-passes-opts                                                                ;;; an-jvm
