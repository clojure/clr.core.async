...
(defn aset-object [^IAtomicReferenceArray arr ^long idx o]                         ;; AtomicReferenceArray
  (.set arr idx o))

(defn aget-object [^IAtomicReferenceArray arr ^long idx]                           ;; AtomicReferenceArray
  (.get arr idx))
...
(defn- emit-state-machine [machine num-user-params custom-terminators]
  (let [index (index-state-machine machine)
    ...
    `(fn state-machine#
	   ([] (aset-all! (conc/->AtomicReferenceArray (object-array ~state-arr-size))                                    ;;; (AtomicReferenceArray. ~state-arr-size)  -- use an Array for now  TODO -- fix!
