...
(defmacro defblockingop
  ...
  (let [as (mapv #(list 'quote %) arglist)]
    `(def ~(with-meta op {:arglists `(list ~as) :doc doc})
       (if (= "true" (when-let [str# (Environment/GetEnvironmentVariable "clojure.core.async.go-checking")] (.ToLower str#)))     ;;;  (Boolean/getBoolean "clojure.core.async.go-checking")
         (fn ~arglist
...
(defn- random-array
  [n]
  (let [rand (System.Random.)                     ;;; (ThreadLocalRandom/current)  if we were in .Net6, we could use Random.Shared.  For backwards compatiblity, we just create a new random.  It's local to this call, so shoudl be thread-safe.
        a (int-array n)]
    (loop [i 1]
      (if (= i n)
        a
        (let [j (.Next rand (inc i))]             ;;; .nextInt
          (aset a i (aget a j))
          (aset a j i)
          (recur (inc i)))))))
...
(defn- pipeline*
  ([n to xf from close? ex-handler type]
     (assert (pos? n))
     (let [ex-handler (or ex-handler #_(fn [ex]                                                ;;; Just ignore this -- no equivalent
   ...
   (take! [_ fn1]
     (let [ret
       (impl/take! ch
         (reify
          ILock                                                                  ;;; Lock
          (lock [_] (.lock ^ILock fn1))                                          ;;; ^Lock 
          (unlock [_] (.unlock ^ILock fn1))                                      ;;; ^Lock 
               ...
               (if (not (nil? v))
                 (let [new-itm (f v)]
                   (if (or (= new-itm last)
                     ...
                     (do (.Add ^ArrayList lst v)                              ;;; .add 
                         (recur lst new-itm))
                     (do (>! out (vec lst))
                         (let [new-lst (ArrayList.)]
                           (.Add ^ArrayList new-lst v)                        ;;; .add 
                           (recur new-lst new-itm)))))
                 (do (when (> (.Count ^ArrayList lst) 0)                      ;;; .size
                       (>! out (vec lst)))
                     (close! out))))))
       out)))