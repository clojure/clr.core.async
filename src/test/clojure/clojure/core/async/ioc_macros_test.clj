                  ...
                  (loop []
                    (try
                      ...
                      (catch ExceptionInfo _
                        :retry))))))
    (is (thrown? ArgumentException                                               ;;; Throwable   -- Normally we replace Throwable with Exception, but that would be just the case above, trying a different exception
                 (runner
                  (loop []
                    (try
                      (pause (throw (ArgumentException. "Ex")))                  ;;; Throwable.
                      (catch ExceptionInfo _
                        :retry))))))
    ...
    (is (= :Exception                                                            ;;; :Throwable
           (runner
            (try
              ...
              (catch ArgumentException _e                                        ;;; Exception
                :ArgumentException)                                              ;;; :Exception
              (catch Exception _t                                                ;;; Throwable
                :Exception))))))                                                 ;;; :Throwable
...
(deftest runner-tests
  ...
  (testing "dot forms"
    (is (= 42 (runner (. Int64 (Parse "42")))))                      ;;; (. Long (parseLong "42"))
    (is (= 42 (runner (. Int64 Parse "42")))))                       ;;; (. Long parseLong "42")
  ...
  (testing "try"
    ...
    (is (= 42
           (runner
            (try
              (assert false)
              (catch Exception ex 42)))))                        ;;; Throwable
   ...
   (let [a (atom false)
          v (runner
             (try
               (assert false)
               (catch Exception _ex true)                        ;;; Throwable
    ...
    (testing "can get from a catch"
      (let [c (identity-chan 42)]
        (is (= 42
               (<!! (go (try
                          (assert false)
                          (catch Exception ex (<! c))))))))))                            ;;; Throwable
  ...
  #_(deftest close-on-exception-tests                                       --- not sure what else to do with this -- we don't ahve uncaughtExceptionHandler
...
(deftest go-propagates-primitive-hints
  (is (= "asd" (<!! (let [a (int 1)] (go (.Substring "fasd" a))))))                          ;;; .substring
  #_(is (= 1 (<!! (let [a (int 1)] (go (Integer/valueOf a))))))                              ;;; not sure what the equivalent is here.
  (is (= [1 1] (<!! (x (R. 1))))))
  ...
  (let [resp (runner
              (try
                ...
                (catch ExceptionInfo ex
                  ...
                  :ok)
                (catch InvalidOperationException ex                                          ;;; UnsupportedOperationException
                  :unsupported)))]
    (is (= :ok resp))))
